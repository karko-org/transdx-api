...
- `cases` — 진단 케이스, 상태: `draft → diagnosed → quoted → completed`. `mileage Int?` (방문 시점 주행거리 스냅샷), `memo String?` (상담 메모) 포함 (5e-a.0 적용 2026-04-24).
...
`src/server.ts`에 등록되는 라우트:

### 시스템
- `GET /api/health`

### 관리자 (admin token, requireAdmin preHandler)
...
### 상담자 (counselor token, requireCounselor preHandler) — Step 5e 추가
- `POST /api/auth/login`, `GET /api/auth/me` — `src/routes/counselorAuthRoutes.ts`
- `GET    /api/cases?status=...` — 목록 (workshop 스코프, 쉼표 구분 status 필터)
- `POST   /api/cases` — 생성 (vehicle upsert를 transaction으로 함께 처리)
- `DELETE /api/cases/:id` — 삭제 (cascade 자동, hard delete)
- `GET    /api/vehicles?plate_number=...` — lookup (404 가능)

**주의 1:** 마스터 데이터(symptom/category/question/symptom-question/diagnosis-rule) 라우트는 전부 한 파일 `adminMasterSymptomRoutes.ts`에 통합되어 있다. 파일명만 보고 없다고 판단하지 말 것.

**주의 2 — 두-토큰 의도적 분리**: `/api/admin/login`은 joey 혼자 쓰는 웹 패널(transdx-admin-console)용으로 마스터 데이터 CRUD 권한. `/api/auth/login`은 공업사 상담자들이 쓰는 모바일 앱(transdx-mobile)용으로 케이스 런타임만. 보안 경계, 토큰 정책을 다르게 가져가기 위한 의도적 분리 — "왜 두 벌이지?"라고 통합 판단하지 말 것. JWT payload의 `type` 필드로 토큰 종류를 구분 (admin token: type 미지정, counselor token: `type='counselor'` + `user_id` + `workshop_id`).

추가 예정 (Week 4+):
- 공개 마스터 데이터 조회 (앱에서 증상/질문 읽기)
- 케이스 런타임 확장 (증상 선택, 질문 응답, diagnosis run 실행, quote 생성)
...
- 인증은 preHandler로 `[app.requireAdmin]` (관리자 라우트) 또는 `[app.requireCounselor]` (상담자 라우트) — `src/plugins/auth.ts`에서 decorate
  - `requireAdmin`: JWT verify → DB `role='admin' && is_active` 확인 → `request.adminUser` 주입
  - `requireCounselor`: JWT verify → token `type='counselor'` 체크 → DB `role ∈ {counselor, manager} && is_active` 확인 → `request.counselor` 주입
  - Cross-boundary access (admin token으로 counselor route, 또는 반대) → 403
...
9. counselorUser (`.env`의 COUNSELOR_SEED_USERNAME/PASSWORD — 미설정 시 silent skip, 5e-a.2 추가)
...
- ✅ **Case.mileage** + **Case.memo** (둘 다 `Int? / String?` nullable) — 2026-04-24 schema 적용 (5e-a.0, commit a6b4e45). 모바일 신규 접수 화면에 mileage 입력 필드 노출, memo는 백엔드 입력만 (현재 UI 미노출, 향후 진단 결과 화면에서 활용 가능).

- ✅ **L-04 분리 여부** (2026-04-21 해소) — 노션 1번 중분류 최신본에서
  입력축(L-04)/출력축(L-05) 분리 확정. M01 seed 작업에 반영 완료.

- ✅ **counselor seed** (2026-04-24 적용) — 5e-a.2에서 `seedCounselorUser` 구현 + seed 파이프라인 등록. CLAUDE.md에 documented but absent였던 drift 해소 (commit baff06d).
...
## 주차 컨텍스트 (마지막 갱신: 2026-04-27)

> **갱신 룰**: Week 전환 시 이 섹션을 갱신할 것. 오래 방치되면
> Claude Code에게 거짓말이 됨.

- ✅ Week 1 Track B — 레포 세팅, 스키마, Docker
- ✅ Week 2 — 관리자 마스터 데이터 CRUD
- ✅ **Week 3 Track B 완료 (2026-04-27)** — 상담자 인증 + cases/vehicles CRUD 백엔드 인프라 (Step 5e). 모바일 login → list → create 플로우가 이 백엔드 위에서 실 데이터 E2E로 동작. 9 curl 검증 통과.
- ▶ Week 4 — 진단 엔진 (M01 범위, 증상 선택 + 질문 응답 + diagnosis_runs 생성 + 후보 산출)